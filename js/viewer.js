// BAR Model Viewer - Three.js Script
// Paste this as an <script type="module"> block in Webflow (after the importmap).
// Also requires: <script type="importmap"> with three@0.160.0
...
// FIX: gebruik word-boundary matching voor hide-keywords zodat 'glow' niet
// matcht in 'legLowerL1' (le-glow-er). Een keyword matcht alleen als het
// voorafgegaan en gevolgd wordt door een niet-letter teken of string-grens,
// OF als het een hoofdletter-grens heeft (camelCase split).
const HIDE_KEYWORDS = ['flare','fire','emit','jam','wake','bow','nano','blink','glow','light'];

function shouldHideMesh(name) {
    // Splits camelCase/PascalCase naam in losse woorden, check elk keyword
    // Voorbeeld: 'legLowerL1' -> ['leg','Lower','L','1'] -> geen match op 'glow'
    //            'rocketFlare1' -> ['rocket','Flare','1'] -> match op 'flare'
    ...
    Promise.all([
        ...
        modelScene.traverse(child => {
            if (child.isMesh) {
                ...
                // FIX: gebruik word-boundary matching — 'glow' mag niet matchen in 'legLowerL1'
        ...
        window.addEventListener('mousemove', (e) => { if (isAltDown) lightGroup.rotation.y += e.movementX * 0.01; });
